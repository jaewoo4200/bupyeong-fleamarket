...
- **현장 명단 편집**: `Store.addSeller/updateSeller/removeSeller`(양 어댑터, `updateSeller`는 `productText` 변경 시 `categoryKey` 재계산)로 셀러 추가·수정·삭제. UI는 관리자 명단 탭 `RosterTable`(추가 폼 + 행별 인라인 수정/삭제, 2매대 즉시 토글).
- **엑셀 업로드(명단)**: `parseSellersWorkbook`이 번호·상호·이름·취급상품·2매대 + **상단 셀의 행사 날짜**(`ParseResult.date`, raw 일련번호→`SSF`)를 인식. `Store.importSellersForDate(date, rows)`가 그 날짜 행사를 찾거나(없으면 `weekdayFromDate`로 새 행사 생성) 명단 일괄 적용 + 현재 선택. 날짜 없으면 `importSellers(eventId, rows)`로 폴백.
