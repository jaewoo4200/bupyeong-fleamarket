...
- 검증: typecheck·build·test(17/17). 로컬 프리뷰 E2E — 추가(41팀, ‘향수 및 비누’→향수·비누 자동분류)·수정(상호/취급상품→카테고리 wood 재계산/2매대 on)·삭제(40팀 복귀) 확인, 콘솔 에러 0.

## 2026-06-30 (Codex) — 상호명 엑셀 동기화 + 선 없는 셀러 티켓
- **상호명 검증/수정**: `_input/2026년 플리마켓 자리배치도_최종.xlsx`의 `플리마켓` 시트 병합셀 라벨 84개와 `LANDMARKS` 84개를 대조. `네셔널 지오그라피`, `투 파인더 피터`, `그날의 온도`, `우리들 안경`, `성현사주 타로카페` 등 기존 `·` 표기 라벨을 엑셀 공백 표기로 동기화.
- **표시 정규화**: `SeatMap.normalizeMapLabel`은 엑셀의 실제 단어 공백은 보존하고, `문 화 의 거 리`, `배 전 반`, `웰 메 이 드`처럼 글자 단위로 띄어진 운영/구조물 라벨만 화면에서 자연스럽게 붙여 표시.
- **셀러명 표시 변경**: 기존 좌석 주변 연결선 콜아웃을 제거하고, 선 없는 티켓형 배지로 교체. 배지는 카테고리 색상의 좌석번호 칩 + 큰 상호명 텍스트로 구성해 좌석 번호와 셀러명이 함께 읽히되 지도 위 선이 어지럽게 보이지 않도록 변경.
- **적용 범위 확인**: `SeatMap` 직접 렌더는 `SeatMapPanel` 내부 1곳뿐. `/admin`, `/board`, `/`, `SeatConfigMap`은 `VenueMapViewer → SeatMapPanel`, `/seller`, `/kiosk`는 `SeatMapPanel`을 직접 사용하므로 이번 지도 변경은 전체 지도 사용처에 공통 반영.
- **회귀 테스트 추가**: `tests/venue-layout.test.ts` 추가. 원본 엑셀 병합셀 상호/랜드마크 라벨과 코드의 `LANDMARKS` 라벨을 정렬 비교해 이후 상호명 어긋남을 자동 감지.
- **검증**: `npm run typecheck` 통과, `npm test` 17/17 통과(정상 권한), `npm run build` 성공(정상 권한). `npm run lint`는 기존 unrelated React lint 오류 4건(`AdminGate`, `BuskingPanel`, `StaffPanel`, `SeatRoulette`)으로 계속 실패.

## 2026-06-30 (Codex) — 자리배치도 운영지도형 리디자인
- **상가/건물명 표시**: `SeatMap`에 `normalizeMapLabel`을 추가해 원본 라벨의 `·`와 한글 사이 `.`를 화면 표시에서 제거. 상가·건물명은 줄임표 없이 회전/다중행/가벼운 `textLength`로 전부 표시하도록 변경.
- **지도 시각 구조**: 원본 좌표/좌석 배치는 유지하면서 거리 배경, 상가 카드, 좌석 배지, 하이라이트, 나무매대 바, 벤치점을 더 굵고 또렷하게 조정. 기존 크림/코랄/카테고리 컬러 톤은 유지.
- **셀러명 표시 혁신안**: 좌석 내부에 `번호+상호명`을 같이 넣지 않고, 좌석 번호는 크게 유지한 채 배정 셀러명은 큰 콜아웃 라벨(카테고리 색상 바/연결선)로 분리. 라벨 폰트/높이/간격을 추가 상향해 번호와 셀러명이 서로 작아지지 않게 구성.
- **공간 확대**: `SeatMapPanel`의 `preview | standard | large` 캔버스 min-width를 더 키워 PC/태블릿에서는 더 크게 보이고, 작은 화면에서는 가로 스크롤/크게 보기 모달로 선명도를 유지.
- **브라우저 확인**: 개발 서버 `http://localhost:3001`의 `/board` 기본 화면에서 상가명, 좌석 번호, 배정 셀러 콜아웃 표시 확인. 이후 Chrome이 사용자 Claude 탭으로 전환되어 추가 모달 조작은 중단.
- **정적 검증**: `npm run typecheck` 통과, `npm test` 16/16 통과(정상 권한), `npm run build` 성공(정상 권한). `npm run lint`는 기존 unrelated React lint 오류 4건(`AdminGate`, `BuskingPanel`, `StaffPanel`, `SeatRoulette`)으로 계속 실패.

## 2026-06-30 (Codex) — 지도 텍스트 정리 + 배정 셀러 콜아웃 표시
- **셀러명 표시 방식 변경**: `SeatMap`에서 배정 좌석 내부에 `번호+셀러명`을 세로로 압축하던 방식을 제거. 좌석 칸 안에는 번호를 크게 유지하고, 셀러명은 좌석 주변의 흰색 콜아웃 라벨(카테고리 색상 바/연결선)로 표시해 두 정보가 서로 작아지지 않게 변경.
- **라벨 가독성 정리**: 상가/건물명은 과도한 `textLength` 압축을 피하고, 가능한 경우 폰트를 살짝 줄여 전체 이름을 보존. 그래도 공간이 부족한 좁은 박스만 줄임표 처리. 세로형 상가 박스는 더 자연스럽게 회전 라벨로 처리.
- **톤앤매너**: 기존 크림/코랄/카테고리 컬러 톤을 유지하고, 콜아웃은 흰 배경+얇은 컬러 라인으로 지도 위 정보층처럼 보이게 구성.
- **브라우저 확인**: 개발 서버 `http://localhost:3001`의 `/board`에서 윗구간/아랫구간 기본 지도와 아랫구간 크게 보기 모달 확인. 좌석 번호가 크게 유지되고, `보아야쥬`, `비틴`, `쿠잉펫`, `한계플랜트`, `로즈피크닉...` 등 배정 셀러명이 별도 라벨로 보이는 것 확인.
- **정적 검증**: `npm run typecheck` 통과, `npm test` 16/16 통과, `npm run build` 성공. `npm run lint`는 기존 unrelated React lint 오류 4건(`AdminGate`, `BuskingPanel`, `StaffPanel`, `SeatRoulette`)으로 계속 실패.

## 2026-06-30 (Codex) — 건물명/상가명 라벨 복구 + 지도 글자 추가 확대
- **복구**: `lib/venue/bounds.ts`에서 좌석 세로 범위와 겹치는 랜드마크만 남기던 필터를 제거하고, 원본 엑셀 기반 `LANDMARKS` 전체(상가/건물명, 포장마차, 문화의거리입구, 조형물 등)를 다시 `VISIBLE_LANDMARKS`로 렌더. 지도 bounds도 전체 라벨 기준으로 계산.
- **가독성**: `SeatMap`의 상가/건물명·구조물 라벨과 좌석 번호 폰트 크기를 추가 상향하고, 라벨 텍스트 대비를 강화. 기존 `textLength`/폭 추정 압축 방식은 유지해서 긴 라벨과 결합석 번호 잘림을 계속 방지.
- **문서**: `CLAUDE.md`의 지도 라벨/크롭 설명을 현재 동작(원본 엑셀 라벨 전체 표시)에 맞게 갱신.
- **브라우저 확인**: 개발 서버 `http://localhost:3001`의 `/board`에서 윗구간 `LG U+`, `맥도날드`, `금강제화`, `나이키` 등과 아랫구간 `카리타스`, `미도인`, `명동의류`, `타로카페`, `포장마차`, `문화의거리입구` 등 건물/상가명이 크게 복구된 것 확인.
- **정적 검증**: `npm run typecheck` 통과, `npm test` 16/16 통과, `npm run build` 성공. `npm run lint`는 기존 unrelated React lint 오류 4건(`AdminGate`, `BuskingPanel`, `StaffPanel`, `SeatRoulette`)으로 계속 실패.
